...
class DAO:
    ...
    def show_processor(self, id):
        CPU = ProcessorDAO()
        self.print_response(CPU.table, CPU[id].get_record(), CPU.showFields())

    def show_processors(self):
        CPU = ProcessorDAO()
        self.print_responses(CPU.table, CPU.get_all(), CPU.showFields())

    def show_basket(self, id_client):
        basket = BasketDAO().get_porc_from_basket(id_client)
        titles = ['id', 'name', 'price', 'quantity']
        self.print_responses(BasketDAO().table, basket, titles)

    def add_to_basket(self, id_cpu, id_client):
        BasketDAO().check_in_the_clients(id_client)
        BasketDAO().check_in_the_processors(id_cpu)
        BasketDAO().add_to_basket(id_cpu, id_client)

    def remove_from_basket(self, id_cpu, id_client):
        BasketDAO().check_in_the_clients(id_client)
        BasketDAO().check_in_the_processors(id_cpu)
        BasketDAO().remove_from_basket(id_cpu, id_client)
    ...
    def buy(self, id_client):
        ...
        if client[id_client].enough_balance(basket.get_price(id_client)):
            if basket.check_enough_on_stocks(id_client):
                OrderDAO().end_order(id_client)
                client[id_client].pay(basket.get_price(id_client))
                basket.write_off_processors(id_client)
                print('Successfully')
            else:
                self.print_error('insufficient processors in the stocks')
        else:
            self.print_error('insufficient funds in the account')
...
class OrderDAO(DAO):
    ...
    def end_order(self, id_client):
        basket = BasketDAO().get_basket(id_client)
        if basket:
            with self.conn.cursor() as cursor:
...
class BasketDAO(DAO):
    ...
    def check_enough_on_stocks(self, id_client):
        ...
        flag = True
        for i in basket:
            if not ProcessorDAO().check_balance(i[0], i[-1]):
                flag = False
                break
        return flag
        ...
        with self.conn.cursor() as cursor:
            ...
            if responce:
                ...
                if responce:
                    ...
                    OrderDAO().append_new_order()
                    self.append({'id_processor': id_cpu, 'quantity': 1,
                                 'id_client': id_client, 'id_order': OrderDAO().get_last_order()})
